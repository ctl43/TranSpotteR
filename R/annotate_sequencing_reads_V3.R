...
.annotate_reads <- function(seq){
  if(length(seq)==0){
    return(NULL)
  }
  # Mapping to LINE1 sequence
  aln_1 <- bwa_alignment(seq, ref = "~/dicky/reference/fasta/line1_reference/hot_L1_polyA.fa", 
                         samtools_param = "")
  ...
  aln_2 <- mclapply(clipped_seq_1, bwa_alignment, call_bwa ="bwa mem", samtools_param = "-F 128 -F 4")
...
annotate_constructed_reads <- function(clusters, partner_is_anchor=FALSE){
  cluster_anno <- .annotate_reads(unlist(clusters$cluster_contigs))
  partner_anno <- .annotate_reads(unlist(clusters$partner_contigs))
  long_anno <- .annotate_reads(unlist(clusters$long_contigs))
  ...
  # clusters$has_anchor <- rowSums(matrix(apply(mapping2genome, 2, lengths), ,ncol(mapping2genome)))>0 # Need optimisation
  ...
  combined_anchor <- split(combined_anchor, factor(gsub("\\..*","", names(combined_anchor)), levels = rownames(mapping2genome)))
  ...
  if(strand=="+"){
    combined_storage[both_mapped] <- (mapply(function(x,y)c(x,"NNNNN",y), x=anchor_storage[both_mapped], y=partner_storage[both_mapped], SIMPLIFY = FALSE))    
  }else{
    combined_storage[both_mapped] <- (mapply(function(x,y)c(y,"NNNNN",x), x=anchor_storage[both_mapped], y=partner_storage[both_mapped], SIMPLIFY = FALSE))
  }
